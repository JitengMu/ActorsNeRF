...
def cpu_data_to_gpu(cpu_data, exclude_keys=None):
    ...
    for key, val in cpu_data.items():
        ...
        if key == 'data_enc':
            gpu_data['data_enc'] = {}
            for aug_idx, data_enc in cpu_data['data_enc'].items():
                gpu_data['data_enc'][aug_idx] = {}
                for sub_k, sub_val in data_enc.items():
                    if sub_k in exclude_keys:
                        continue
                    gpu_data['data_enc'][aug_idx][sub_k] = sub_val.cuda()
            continue
